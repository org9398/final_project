...
We are utilizing the National Health Interview Survey results from 2022 as one part of our analysis. This survey is conducted each year to capture statistical information on the amount, distribution, and effects of illness and disability in the United States. The units of measurement are individual adults. The survey contains information on immigration demographic characteristics in addition to health related characteristics. Below, we will utilize unsupervised machine learning techniques, specifically dimension reduction, to analyze the relationship between health and demographic characteristics and estimate a model, given that the survey has many correlated predictors.

We used the IPUMS to download the 2022 NHIS data. We read in the data using the ipumsr package.
...
ddi <- read_ipums_ddi("data/nhis_00003.xml")
...
# Data Cleaning and Subsetting for NHIS

# creating vector for missingness/lack of response coded values 
missing_values <- c(0, 00, 000, 7, 8, 9, 97, 98, 99, 900, 997, 998, 999) 
nhis <- nhis %>% 
  mutate_all(~ ifelse(. %in% missing_values, NA, .))

nhis <- nhis %>%
  janitor::clean_names() %>%
  filter(!(health %in% c(7, 8, 9)), 
         !(yrsinusg %in% c(0, 7, 8, 9)),
         (age != 0),
         !(sex %in% c( 7, 8, 9)),
         !(sexorien %in% c(0, 7, 8)),
         !(marstat %in% c(00, 99)),
         !(marst %in% c(00, 99)),
         !(marstcohab %in% c(0, 9)),
         !(marriedev %in% c(0, 7, 8, 9)))
  

nhis$health <- factor(nhis$health, levels = 1:5)
...
Unsupervised Machine Learning using the NHIS
