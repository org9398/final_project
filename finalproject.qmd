...
The below table notes the total population (broken out by immigrants and non-immigrants), median income, an health insurance prevalence by state.
...
acs_by_state %>%
  ...
  group_by(State) %>%
  summarise(State, sum(total_pop), mean(born_us), mean(born_pr), mean(born_abroad), mean(median_income), sum(health_insurance)) %>%
  knitr::kable(digits = 2)
