...
The below analysis begins with an overarching look at the immigrant landscape in the United States, from its geographic distribution at the county level to median income levels and prevelance of health insurance. It then proceeds to unsupervised machine learning followed by supervised machine learning in order to predict the likelihood of immigrant health coverage.

The below table notes the total population (broken out by immigrants and non-immigrants), median income, an health insurance prevelance by state.
...
cps_graph_1 <- cps %>%
  ...
  scale_fill_manual(values = c("1" = "red", "2" = "blue"),
                     labels = c("1" = "Uninsured", "2" = "Insured"))
...
cps_graph_2 <- cps %>%
  mutate(citizen = as.factor(citizen)) %>%
  filter(citizen == 5) %>%
  ggplot(aes(x = healthinsu, fill = healthinsu)) +  # Added fill aesthetic
  geom_bar() +
  geom_text(
    stat = "count", 
...
In an attempt to broadly understand our CPS dataset, we conducted unsupervised machine learning to summarize the data without a set outcome variable.
